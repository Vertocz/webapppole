...
export default function StaffPage() {
  ...
  // Onglets dynamiques selon la catégorie du joueur sélectionné (consultation
  // en lecture seule des données d'un joueur — n'inclut pas "Jeux", qui n'est
  // pas une donnée à consulter mais une session à laquelle le staff participe
  // lui-même : voir viewTabs plus bas).
  const ongletsJoueur = [
    { id: "sportif", label: "Suivi sportif", icon: "⛹️‍♀️" },
    { id: "forme", label: "Forme", icon: "🧘‍♀️" },
    ...(selectedJoueur?.categorie === "Masculin" ? [{ id: "mentale", label: "Prépa mentale", icon: "🧠" }] : []),
  ...
  const pole: "masculin" | "feminin" | "both" = !user
    ? "both"
    : user.masculin && user.feminin
    ? "both"
    : user.masculin
    ? "masculin"
    : "feminin";

  if (!user || !badgesChecked)
    return (
  ...
  const viewTabs: { id: View; label: string; icon: string }[] = [
    ...
    ...(isLogisticien ? [{ id: "gares" as View, label: "Gares", icon: "🚉" }] : []),
  ];
  ...
  return (
    <>
      ...
      <div className="relative min-h-screen z-10">
        ...
        <main className="max-w-3xl mx-auto px-4 py-6 space-y-5">
          <div className="overflow-x-auto scrollbar-hide -mx-4 px-4">
            <div
              className="flex rounded-xl p-1 gap-1 w-max min-w-full"
            ...
            <div className="space-y-5">
              ...
              {selectedJoueur && (
                <div className="animate-fade-in-up" key={selectedJoueur.id}>
                  ...
                  {ongletActif === "mentale" && selectedJoueur.categorie === "Masculin" && (
                    <PreparationMentale userId={selectedJoueur.id} readOnly />
                  )}
                  {ongletActif === "badges" && (
                    <BadgesTab userId={selectedJoueur.id} userType="joueur" categorie={selectedJoueur.categorie} readOnly />
                  )}
                </div>
              )}
            </div>
      ...
      {newBadgeIds.length > 0 && <BadgePopup badgeIds={newBadgeIds} onDone={() => setNewBadgeIds([])} />}
      ...
      {exportOpen && <ExportPDFModal joueurs={joueurs} onClose={() => setExportOpen(false)} />}
