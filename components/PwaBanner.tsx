...
export default function PwaBanner() {
  ...
  useEffect(() => {
    ...
    if (dismissed) return;
    if (isInstalled()) return; // déjà installé, pas besoin

    const detectedOS = detectOS();
    if (detectedOS === "desktop") return; // pas pertinent sur desktop
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 px-4 pb-4 animate-slide-in-up">
      <div className="rounded-2xl p-4 shadow-2xl"
        ...
        <button onClick={dismiss}
          className="w-full mt-3 py-2.5 rounded-xl text-xs font-medium tracking-widest"
          style={{ border: "1px solid var(--border)", color: "var(--text-muted)" }}>
          Ignorer
        </button>
      </div>
  ...
  return (
    <div className="flex items-center gap-2.5">
      ...
      <span className="text-xs" style={{ color: "var(--text-sub)" }}>
        {text} <span className="font-bold" style={{ color: "var(--text-main)" }}>{icon}</span>
      </span>
    </div>
  );
}
