        ...
        return (
          <div key={p.name} className="flex items-center justify-between gap-4">
            ...
            <span className="font-display text-sm" style={{ color: "var(--text-main)" }}>
              {p.value}<span style={{ color: "var(--text-muted)", fontSize: "0.65rem" }}>/5</span>
            </span>
          </div>
        );
      })}
    ...
    <div className="flex items-center gap-1 p-1 rounded-xl" style={{ background: "rgba(255,255,255,0.04)" }}>
      {PERIODES.map((p) => (
        <button key={p.val} onClick={() => onChange(p.val)}
          ...
          style={{
            background: value === p.val ? "var(--primary)" : "transparent",
            color: value === p.val ? "white" : "var(--text-muted)",
            boxShadow: value === p.val ? "0 2px 8px rgba(27,58,140,0.4)" : "none",
          }}>
          {p.label}
        </button>
      ))}
  ...
  const donnees = useMemo(() => {
    ...
    return [...data]
      ...
      .map((d) => ({
        ...
        fatigue: d.fatigue, sommeil: d.sommeil, douleur: d.douleur, stress: d.stress, humeur: d.humeur,
      }));
  }, [data, periode]);
            ...
            <div style={{ height: 220 }}>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={donnees} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <YAxis domain={[0, 5]} ticks={[0, 1, 2, 3, 4, 5]} tick={{ fill: "var(--text-muted)", fontSize: 10 }} tickLine={false} axisLine={false} />
