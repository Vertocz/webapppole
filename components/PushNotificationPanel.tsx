...
export default function PushNotificationPanel({ staffId, pole }: Props) {
  ...
  const loadData = useCallback(async () => {
    // Charger tous les joueurs du scope du staff (filtrage affiné localement via targetPole)
    ...
    // pole === "both" → pas de filtre, tous les joueurs
    const { data: joueusesData } = await q.order("prenom", { ascending: true });
    if (joueusesData) setJoueurs(joueusesData);
  ...
  // Filtre par pôle cible (utile quand pole === "both") puis par recherche
  const filteredJoueurs = joueurs
    ...
    .filter((j) =>
      `${j.prenom} ${j.nom}`.toLowerCase().includes(searchPlayer.toLowerCase())
    );
              ...
              <div className="rounded-xl overflow-hidden" style={{ border: "1px solid var(--border)" }}>
                ...
                <div className="max-h-48 overflow-y-auto" style={{ background: "var(--bg-card)" }}>
                  {filteredJoueurs.length === 0 ? (
                    <p className="text-sm text-center py-4" style={{ color: "var(--text-muted)" }}>Aucun joueur trouvé</p>
                  ) : (
